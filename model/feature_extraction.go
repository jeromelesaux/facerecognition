...
func NewLDA(trainingSet []algorithm.Matrix, labels []string, numOfComponents int) *LDA {
	l := &LDA{FeatureExtraction: &FeatureExtraction{}}
	n := len(trainingSet)
	...
	l.FeatureExtraction.ProjectedTrainingSet = make([]ProjectedTrainingMatrix, len(trainingSet))
	for i := 0; i < len(trainingSet); i++ {
		...
		l.FeatureExtraction.ProjectedTrainingSet[i] = *ptm
	}
...
func NewPCA(trainingSet []algorithm.Matrix, labels []string, numOfComponents int) *PCA {
	...
	// Construct projectedTrainingMatrix
	p.FeatureExtraction.ProjectedTrainingSet = make([]ProjectedTrainingMatrix, len(trainingSet))
	for i := 0; i < len(trainingSet); i++ {
		...
		p.FeatureExtraction.ProjectedTrainingSet[i] = *ptm
	}
	return p
}
...
func (p *PCA) GetFeature(input []algorithm.Matrix, k int) *algorithm.Matrix {
	...
	for i := 0; i < column; i++ {
		x.SetMatrix(0, row, i, i, input[i].Minus(p.FeatureExtraction.MeanMatrix))
	}
	...
	for i := 0; i < column; i++ {
		...
		for j := 0; j < row; j++ {
			selectedEigenVectors.A[j][j] /= temp
		}
	}
